...
def filter_italian_regions(input_path: str, output_path: str) -> str:
    ...
    try:
        ...
        logger.info(
            f"Filtered {count_italian} Italian regions out of {count_total} total regions"
        )
        return output_path
...
def load_regions_to_opensearch(input_path: str,
                               index_name: str,
    ...
    try:
        ...
        client = OpenSearch(
            ...
            ssl_assert_hostname=opensearch_config.get('ssl_assert_hostname',
                                                      False),
        ...
        with open(input_path, 'r', encoding='utf-8') as file:
            for line in file:
                try:
                    ...
                    hotels_number = len(doc.get(
                        'hotels', [])) if doc.get('hotels') else 0
...
def sync_regions_to_opensearch(dump_url: str,
                               index_name: str = 'region_italy_ratehawk',
                               opensearch_config: Optional[Dict[str,
                                                                Any]] = None,
    ...
    try:
        ...
        # 4. Carica in OpenSearch
        doc_count = load_regions_to_opensearch(filtered_file, index_name,
                                               opensearch_config)
